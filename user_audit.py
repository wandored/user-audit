...
def read_ms365():
    ms = pd.read_csv("./downloads/ms.csv", usecols=["mail"])
    return ms
...
def read_r365():
    ...
    #    rackspace = pd.read_csv('./downloads/rackspace.csv')
    return r365


def read_rackspace():
    file_path = glob.glob("./downloads/rackspace/*.csv")
...
def read_reference():
    reference = pd.read_csv(
        "./downloads/centra_ref_users.csv",
        usecols=[
            "user_login",
            "user_email",
            "display_name",
...
def main(master, file, source):
    ...
    print(not_in_file)
    input("Press Enter to continue...")
    # matches.to_csv("./output/users.csv", index=False)
    with pd.ExcelWriter(f"./output/{source}.xlsx") as writer:
